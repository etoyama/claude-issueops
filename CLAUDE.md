...
docs/design/epic-NN-*.md        # Epic Living Design Doc (v0.2 着手時に新設)
```
...
元の `.spec-workflow/specs/` は `.gitignore` で個人 artifacts として ignored。tracked archive snapshot を [`docs/legacy/v0.1-spec-workflow/session-closer/`](./docs/legacy/v0.1-spec-workflow/session-closer/) に置いた。詳細は [`ARCHIVED.md`](./docs/legacy/v0.1-spec-workflow/session-closer/ARCHIVED.md) を参照。
...
指摘の優先度を **high / medium / low** で評価し、**high + medium** だけを当該 PR 内で fix。**low は defer** して次 Story / Epic で再評価。結果は PR description の Self review セクション ([`.github/PULL_REQUEST_TEMPLATE.md`](./.github/PULL_REQUEST_TEMPLATE.md)) に貼る。
